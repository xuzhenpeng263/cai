...
from cai.sdk.agents.tracing import mcp_tools_span
import functools
...
class GlobalMCPUtil(MCPUtil):
    ...
    def to_function_tool(cls, tool, server_name: str) -> FunctionTool:
        ...
        # For STDIO servers, capture the command
            ...
            server_config['encoding_error_handler'] = server.params.get('encoding_error_handler', 'strict')
        ...
        async def invoke_with_fresh_connection(config, context, input_json):
            ...
            from cai.sdk.agents.exceptions import ModelBehaviorError, AgentsException
            ...
            from cai.sdk.agents.tracing import get_current_span, FunctionSpanData
        ...
        invoke_func = functools.partial(
            invoke_with_fresh_connection, 
            server_config
        )
...
"""
    ...
    def handle_load(self, args: Optional[List[str]] = None) -> bool:
        ...
        if args[0] == "stdio":
            if len(args) < 3:
                console.print(
                    "[red]Error: stdio requires name and command[/red]"
                )
                return False
    ...
    def _load_sse_server(self, url: str, name: str) -> bool:
        ...
        try:
            ...
            console.print(
                f"[green]✓ Connected to SSE server '{name}' "
                f"at {url}[/green]"
            )
    ...
    def _load_stdio_server(
        self, name: str, command: str, cmd_args: List[str]
    ) -> bool:
        ...
        console.print(
            f"Starting stdio server '{name}' with command: "
        ...
        try:
            ...
            console.print(
                f"[green]✓ Started stdio server '{name}'[/green]"
            )
            console.print(f"Available tools: {len(tools)}")
    ...
    def handle_add(self, args: Optional[List[str]] = None) -> bool:
        ...
        try:
            ...
            try:
                ...
                console.print(
                    f"[red]Error: Agent '{agent_identifier}' not found[/red]"
                )
                return False
        ...
        console.print(
            f"Adding tools from MCP server '{server_name}' to agent "
        ...
        try:
            async def validate_connection():
                try:
                    ...
                    console.print(f"[yellow]Warning: Server connection may be lost, attempting to reconnect...[/yellow]")
                    # Try to reconnect
                    await server.connect()
                    tools = await server.list_tools()
        ...
        try:
            ...
            async def test_agent_tools():
                ...
                all_mcp_tools = await agent.get_mcp_tools() if hasattr(agent, 'mcp_servers') and agent.mcp_servers else []
                return all_regular_tools + all_mcp_tools
            
            all_tools = self._run_async(test_agent_tools())
            
            # Count different types of tools
            mcp_server_tools_count = len([t for t in agent.mcp_servers if hasattr(agent, 'mcp_servers')]) if hasattr(agent, 'mcp_servers') else 0
            regular_tools_count = len(agent.tools) if hasattr(agent, 'tools') else 0
            
            console.print(
                f"[blue]Agent now has {regular_tools_count} tools total[/blue]"
            )
            ...
            try:
                if tools:
                    console.print(f"[green]✓ MCP tools are ready for use![/green]")
                else:
    ...
    def handle_remove(self, args: Optional[List[str]] = None) -> bool:
        ...
        try:
            ...
            console.print(
                f"[green]✓ Removed MCP server '{server_name}'[/green]"
            )
            return True
        except Exception as e:
    ...
    def handle_status(self, args: Optional[List[str]] = None) -> bool:
        ...
        if healthy_count == total_servers:
            ...
            console.print(f"[yellow]⚠ {healthy_count}/{total_servers} servers healthy, {failed_count} failed[/yellow]")
    ...
    def handle_tools(self, args: Optional[List[str]] = None) -> bool:
        ...
        try:
            ...
            if not tools:
                console.print(
                    f"[yellow]No tools available from '{server_name}'[/yellow]"
                )
                return True
